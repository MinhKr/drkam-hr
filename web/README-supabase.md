...
File đó gộp sẵn 9 file migration theo đúng thứ tự, dán một lần là xong. Supabase chạy cả file trong một
...
| 9 | `migrations/0010_danh_muc_crud.sql` | View đếm + hàm đổi tên + hàm sắp xếp — cần cho màn hình quản lý Danh mục |

**Phải chạy `0001` đầu tiên** — nó tạo hàm kiểm tra mà 8 file sau đều gọi. Sai thứ tự sẽ gặp lỗi
...
Bỏ sót `0010` thì màn hình Danh mục vẫn thêm/ẩn/xoá được nhưng không đổi tên, không kéo sắp xếp được và không hiện số hồ sơ đang dùng.
...
Xong 9 file, vào **Table Editor** (schema `public`) sẽ thấy: `candidates`, `interviews`, `onboardings`,
...
| `0010_danh_muc_crud.sql` | Muốn HR tự sửa **Danh mục** trong app thay vì nhờ kỹ thuật | Thêm một view, hai hàm, một trigger ghi nhật ký. Không thêm cột, không sửa dòng nào. Chạy xong hiện `0010 OK`. |
